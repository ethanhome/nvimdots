...
if [[ "$USE_SSH" -eq "1" ]]; then
	if is_latest; then
		...
		execute "git" "clone" "-b" "0.7" "${CLONE_ATTR[@]}" "git@github.com:ethanhome/nvimdots.git" "${DEST_DIR}"
	fi
else
	if is_latest; then
		...
		warn "You have outdated Nvim installed (< ${REQUIRED_NVIM_VERSION})."
		info "Automatically redirecting you to legacy version..."
		execute "git" "clone" "-b" "0.7" "${CLONE_ATTR[@]}" "https://github.com/ethanhome/nvimdots.git" "${DEST_DIR}"
	fi
fi
